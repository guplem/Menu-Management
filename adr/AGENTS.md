...
This directory holds the Architecture Decision Records for Menu Management.

## What is an ADR?

An ADR records one architectural decision or cross-cutting standard, with its context, the decision, and its consequences. It answers "how do we always do X here": the persistence format (`.tsr` / `.tsm` files), the state-management contract (singleton providers, UI-only provider access), a data-model convention (Freezed models with embedded business logic), the menu-generation algorithm contract.

An ADR is **not** a feature explanation and not a setup guide. How the app works and how to run it belongs in `README.md`. See the "Documentation Organization" section of the root `AGENTS.md` for the full map of what goes where.
...
- Standard library or framework usage, one-off bug fixes, minor refactors, minor UI tweaks within an existing pattern.
