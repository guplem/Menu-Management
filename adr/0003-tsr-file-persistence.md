...
- **Exports that are not JSON** (for example the menu PDF) -- `Persistency.saveBytes()` asks the user where to save through the same `FilePicker` save dialog, then writes raw bytes with `saveBytesToPath()`. `Persistency.supportsFileSaving()` reports whether the device has a save dialog at all, so a caller can warn the user instead of failing silently on iOS/Android. An export writes no `last_session.json` entry, because the app has nothing that reads such a file back. `Persistency.defaultMenuFileName()` takes an `extension` parameter so a `.tsm` save and a PDF export of the same menu propose the same file name, differing only in extension.
