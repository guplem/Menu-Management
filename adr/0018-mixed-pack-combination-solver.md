...
The solver enumerates pack-count vectors `(n_1, ..., n_k)` for the `k` same-unit products, with each `n_i` from 0 to that product's solo pack count (`_simulateProduct(...).packsNeeded`, which already accounts for expiry). The cap uses the full-buy count: when `_simulateProduct` applies the under-buy rule (`underBuy` is true), the solver adds the dropped pack back. The reduced count cannot cover the need, so without that pack a single-product search has no feasible candidate. The solver then returned null for many needs that are not an exact pack multiple (issue #48). Buying more of one product than would cover the whole need alone is never less wasteful, so that is a safe upper bound. The search is the Cartesian product of those ranges. When the number of vectors would exceed `_maxCombinationVectors` (20000), the search is skipped and the best single product from `rankProducts` is returned. Products are sorted internally (ascending pack size, then link, then unit) so the result never depends on the caller's input order.
...
- **PDF export** (`_productOptions` in `shopping_pdf_document.dart`): unlike the card and the copy, the PDF does not call `recommendCombination`. It lists every matching product with `Product.packsNeeded` computed for that product alone, and marks every product that ties for the least waste under `rankProducts`. The reason: the `recommendCombination` result drops every product that the chosen mix leaves out. The reader shops from a printed page with no app to fall back on, so a product missing from an empty shelf needs a next line to try; a combination result cannot promise that for every amount. The mark follows the same tie rule as the on-screen chip (`isBestOption` in `shopping_ingredient.dart`), so the page and the PDF never disagree.
- **Events passed to `rankProducts` from the PDF**: the PDF passes the cooking events of the weeks that the section buys for, not empty events and not the whole menu. This differs from the copy, which passes empty events, because the two show different things. The copy shows one waste-minimal mix, and inside one trip only pack-granularity matters. The PDF shows every product and marks the least wasteful one, and shelf life after opening decides that mark; a trip that buys for two weeks must therefore see both weeks' events. A section that covers the whole menu passes every event.
...
- The PDF export is a third surface that intentionally stays on `rankProducts` instead of `recommendCombination`, so a printed page always lists every product the reader can fall back to. This is a deliberate exception to "the combination solver is additive" above, not a gap left to close.
