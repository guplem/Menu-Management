...
- **Aggregated shopping**: `MultiWeekMenu.allIngredients(recipes:)` counts the ingredients of the whole menu in one walk over all weeks, combining same-unit amounts per ingredient. The walk does not add up per-week totals: a cook event late in week N also buys the food of its leftover meals in week N+1, and a per-week total cannot see those meals (issue #49). Each leftover sub-meal belongs to the nearest earlier cook event of its recipe, and only when the meal is inside that cook event's `maxStorageDays` window. A leftover meal that no cook event feeds is not bought. `allIngredients`, `ingredientSources` and `ingredientMealRequirements` share this walk, so the shopping total of an ingredient equals the sum of its per-meal entries. Requires a recipe list since `Cooking` stores recipe IDs, not full objects (see ADR 0009).
