...
- Dynamic-collection providers expose a symmetric API: `get(id)` (using `firstWhereOrNull` + `Debug.logError` + null assertion), a list getter (`.ingredients` / `.recipes`), `setData()` for bulk replacement, and static `addOrUpdate()` / `remove()` for mutation.
