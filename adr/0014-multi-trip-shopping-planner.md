...
`ownedAmountInUnit` converts the same owned stock into each unit independently. When one ingredient is needed in two units at once (for example grams in one recipe and pieces in another, with a pieces product so the normalizer keeps both), subtracting the full stock from each unit over-subtracts. The on-screen "remaining" now runs through `computeRemainingQuantities` (in `owned_amount.dart`), which turns the owned stock into a single grams pool and draws it down across the required units once, so a single stock is never subtracted more than once.
