...
- **Subagents stay at `.claude/agents/`**: `pattern-scout`, `adr-checker`, `docs-checker`, `test-runner`, `research-agent`, `research-synthesizer`.
...
**Verification uses the existing `test-runner` agent, not a separate `validate` agent.** The shared standard names a generic `validate` agent, but this repo already had `test-runner` doing that job. It was kept and extended with a format-check step so it mirrors CI exactly. Adding a second verification agent would duplicate the same role.
