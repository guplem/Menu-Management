...
class Debug {
  ...
  static void log(
    String message, {
    String signature = "🪵 ",
    int ignoredFirstStackTraceRows = 2,
    int maxStackTraceRows = 1,
    bool printInRelease = false,
    ColorsConsole messageColor = ColorsConsole.standard,
  }) {
  ...
  static void logSuccess(
    ...
    String signature = "✔ ",
    int maxStackTraceRows = 1,
    int ignoredFirstStackTraceRows = 3,
    bool showFullStackTrace = false,
    ColorsConsole messageColor = ColorsConsole.green,
  }) {
    log(
      message,
      signature: signature,
      ignoredFirstStackTraceRows: ignoredFirstStackTraceRows,
  ...
  static void logSuccessUpload(
    String message, {
    String signature = "⬆️",
    int maxStackTraceRows = 1,
    int ignoredFirstStackTraceRows = 3,
    bool showFullStackTrace = false,
    ColorsConsole messageColor = ColorsConsole.blue,
  }) {
    log(
      message,
      signature: signature,
  ...
  static void logSuccessDownload(
    String message, {
    String signature = "⬇️",
    int maxStackTraceRows = 1,
    int ignoredFirstStackTraceRows = 3,
    bool showFullStackTrace = false,
    ColorsConsole messageColor = ColorsConsole.cyan,
  }) {
    log(
      message,
      signature: signature,
  ...
  static void logUpdate(
    ...
    int ignoredFirstStackTraceRows = 3,
    bool showFullStackTrace = false,
    ColorsConsole messageColor = ColorsConsole.white,
  }) {
    log(
      message,
      signature: signature,
  ...
  static void logWarning(
    ...
    String message, {
    String signature = "⚠️",
    int maxStackTraceRows = 4,
    bool asAssertion = true,
  }) {
    bool useAssertion = asAssertion && kDebugMode;
    if ((useAssertion && showIf) || showIf) {
      String msg =
          "${signature}Warning: ${_getMessageWithStackTrace(message, maxStackTraceRows: useAssertion ? 0 : maxStackTraceRows)}";
      if (useAssertion) {
        assert(!showIf, msg);
      } else if (showIf) {
  ...
  static void logError(
    String message, {
    String signature = "‼️",
    int maxStackTraceRows = 5,
    bool showFullStackTrace = false,
    bool asException = true,
  }) {
    String msg =
        "${signature}ERROR: ${_getMessageWithStackTrace(message, maxStackTraceRows: asException ? 0 : maxStackTraceRows)}";
  ...
  static String _getMessageWithStackTrace(
    ...
    for (
      int i = ignoredFirstStackTraceRows;
      i < lines.length && i < maxStackTraceRows + ignoredFirstStackTraceRows;
      i++
    ) {
      ...
      if (!showFullStackTrace) {
        ...
        callLoc =
            "${splitted.lastOrNull} [${splitted.first.split("/").lastOrNull} ${splitted[1]}]\n";
      } else {
        callLoc = "${list[i]}\n";
      }
