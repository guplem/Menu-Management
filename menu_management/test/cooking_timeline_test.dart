...
Recipe _recipe({required String id, required String name, required List<Instruction> instructions, int maxStorageDays = 6}) {
  ...
  group("buildCookingTimeline", () {
    test("single recipe in single week produces one event", () {
      ...
      MultiWeekMenu menu = MultiWeekMenu(
        weeks: [
          Menu(
            meals: [_meal(weekDay: WeekDay.monday, mealType: MealType.lunch, recipeId: "r1")],
          ),
        ],
      );
    ...
    test("storable recipe served at multiple meals produces one event with summed people", () {
      Recipe recipe = _recipe(
        id: "r1",
        name: "Stew",
        instructions: [_instruction(ingredientId: "potato", amount: 200)],
      );
    ...
    test("non-storable recipe appearing twice in same week produces two events", () {
      Recipe recipe = _recipe(
        id: "r1",
        name: "Salad",
        instructions: [_instruction(ingredientId: "lettuce", amount: 50)],
        maxStorageDays: 0,
      );
      MultiWeekMenu menu = MultiWeekMenu(
        weeks: [
    ...
    test("two recipes using same ingredient on same day are merged", () {
      ...
      Recipe r2 = _recipe(
        id: "r2",
        name: "Soup",
        instructions: [_instruction(ingredientId: "tomato", amount: 80)],
      );
      MultiWeekMenu menu = MultiWeekMenu(
        weeks: [
          Menu(
    ...
    test("multi-week menu produces events across weeks with correct dayIndex", () {
      Recipe recipe = _recipe(
        id: "r1",
        name: "Pasta",
        instructions: [_instruction(ingredientId: "tomato", amount: 100)],
      );
      MultiWeekMenu menu = MultiWeekMenu(
        weeks: [
          Menu(
          ...
          Menu(
            meals: [_meal(weekDay: WeekDay.wednesday, mealType: MealType.lunch, recipeId: "r1")],
          ),
          Menu(
            meals: [_meal(weekDay: WeekDay.friday, mealType: MealType.lunch, recipeId: "r1")],
          ),
        ],
      );
        ...
        instructions: [
          ...
          Instruction(
            ...
            ingredientsUsed: [
              IngredientUsage(
                ingredient: "tomato",
                quantity: const Quantity(amount: 2, unit: Unit.centiliters),
              ),
            ],
          ),
        ],
