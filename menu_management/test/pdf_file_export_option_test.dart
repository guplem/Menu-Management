...
void main() {
  ...
  PdfFileExportOption option({required String? savedPath, bool canSaveFiles = true}) {
    return PdfFileExportOption(
      label: "PDF",
  ...
  group("PdfFileExportOption", () {
    ...
    test("says that the device cannot save, and builds no bytes, when there is no save dialog", () async {
      ...
      expect(message, PdfFileExportOption.unavailableMessage);
      expect(buildCount, 0);
      expect(saveCalls, isEmpty);
    });

    test("carries the icon and the failure note of a format that writes a file", () {
      PdfFileExportOption pdfOption = option(savedPath: null);
