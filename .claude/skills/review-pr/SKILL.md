...
Record: which areas the PR touches (the feature domains under `menu_management/lib/`: `ingredients/`, `recipes/`, `menu/`, `shopping/`, `theme/`, `flutter_essentials/`, plus `test/`), the total scope (additions/deletions, file count), the stated intent (commits + PR description), and any implementation proposal from the issue discussion.
...
1. **Pattern & architecture.** Does new code sit and register where similar code does? Find two or three existing examples of each new kind of thing and compare. Read files beyond the diff to see how the codebase does it elsewhere. Check the three-layer structure (UI widgets -> providers -> Freezed models), UI-only provider access, and models never importing providers.
...
3. **Quality, safety & security.** Explicit types on public APIs and return types; error handling (no silently swallowed errors); no dead code, no over-engineering, no unnecessary duplication; double quotes and package imports as `analysis_options.yaml` requires; regenerated `*.freezed.dart` / `*.g.dart` committed when a model changed.
...
- **validate** (always): run the repo's checks (format check, `flutter analyze`, `flutter test`) for the touched areas and report each as pass or fail, with failing output verbatim.
- **pattern-scout** (only when the diff adds a new data model, provider, screen, or widget): find how similar ones are built here and report the convention with real examples.
...
- **Findings index**: a numbered list of every finding that survived phase 4, so the user sees the total before the questions begin. Example:
  ```
  1. [Required]   Yields not recalculated on recipe swap - menu_management/lib/menu/menu.dart:120 - fix in this PR
  2. [Suggestion] Missing test for empty shopping list - menu_management/test/shopping_test.dart - fix in this PR
  3. [Suggestion] Pre-existing: duplicated normalize call - follow-up issue
  ```
